(function (window) {
    ...
    c3.generate = function (config) {
        ...
        var __legend_show = getConfig(['legend', 'show'], true),
            __legend_item_width = getConfig(['legend', 'item', 'width'], 105), // TODO: auto
...
//            return d3.min(targets, function (t) { return d3.min(t.values, function (v) { return v.x; }); });
            return targets[0].values[0].x;
        }
        function getXDomainMax(targets) {
//            return d3.max(targets, function (t) { return d3.max(t.values, function (v) { return v.x; }); });
            if (targets.length === 1) {
                return targets[0].values[targets[0].values.length - 1].x;
            } else if (targets.length === 2) {
                return targets[1].values[targets[0].values.length - 1].x;
            }
        }
        ...
        function showTooltip(selectedData, mouse) {
            ...
            tooltip
                .style("top", tooltipTop-45 + "px")
                .style("left", tooltipLeft+30 + 'px')
        ...
        */
        ...
        function init(data) {
            ...
            // Define regions
            //svg.attr("style","margin-left: 20px");
            main = svg.append("g").attr("transform","translate(58,0)");//translate.main);
...
*/
        ...
        function updateLegend(targets, options) {
            ...
            l.append('rect')
                ...
                .style('fill-opacity', 0)
                .attr('x', -200)
                .attr('y', -205)//function () { return legendHeight / 2 - 16; })
            ...
            l.append('rect')
                ...
                .attr('x', -200)
                .attr('y', -205)// function () { return legendHeight / 2 - 9; })
                .attr('width', 10)
                .attr('height', 10);
            l.append('text')
                ...
                .style("pointer-events", "none")
                .attr('x', -200)
                .attr('y', -196)// function () { return legendHeight / 2; });
